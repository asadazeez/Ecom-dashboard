...
const Category = ({categoryList}:Props) => {
  ...
  async function deleteCategory (categoryId:any){
    try{
    const deleteCategory = await categoryApi.deleteCategory(categoryId)
          ...
          <tbody>
            {categoryList.map((categoryItem:any, index : any) => (
              <tr key={index}>
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pr-7.5 ${index === categoryItem.length - 1 ? "border-b-0" : "border-b"}`}
                >
